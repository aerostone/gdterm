    ...
    public class KeePassManagerForm : Form
    {
        ...
        private void LoadEntries()
        {
            ...
            try
            {
                ...
                {
                    var item = new ListViewItem(entry.Title ?? "(无标题)");
                    item.SubItems.Add(entry.Username ?? "");
                    item.SubItems.Add(entry.GroupPath ?? "");
                    item.SubItems.Add(""); // KeePassEntrySummary 不含 URL，留空
                    item.SubItems.Add(""); // KeePassEntrySummary 不含 LastModified，留空
                    item.Tag = entry.Id;
        ...
        private void OnEditClick(object sender, EventArgs e)
        {
            ...
            // 由于 KeePassEntrySummary 不含完整信息，提示用户
            MessageBox.Show(this, "编辑功能需要重新加载完整条目数据。\n当前版本仅支持查看和复制操作。",
